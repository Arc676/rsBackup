...
fn task_editor(ui: &Ui, cfg: &mut TaskConfig) -> bool {
    ...
    ui.text("Task ID:");
    ui.same_line(0.0);
    ui.input_text(im_str!("##ID"), &mut cfg.id).build();

    ui.checkbox(
        im_str!("Always ask for confirmation"),
        &mut cfg.always_confirm,
    );

    ui.text("Source path:");
    ui.same_line(0.0);
    ui.input_text(im_str!("##SRC"), &mut cfg.src).build();

    ui.text("Destination path:");
    ui.same_line(0.0);
    ui.input_text(im_str!("##DST"), &mut cfg.dst).build();
        ...
        .build(&ui, || {
            ...
            if CollapsingHeader::new(im_str!("Saved Tasks")).build(&ui) {
                ...
                if let Some((idx, act)) = action {
                    match act {
                        TaskButtons::RemoveTask => {
                            state.remove_task_at(idx);
                        }
                        _ => {
                            state.edit_task_at(idx);
                        }
                    };
                }
            }
